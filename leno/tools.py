...
def tool_run_shell(context, args):
    ...
    if os.name == "nt":
        ...
        git_sh = Path(git_path).resolve().parent.parent / "usr" / "bin" / "sh.exe" if git_path else None
        shell_command = [str(git_sh), "-c", command] if git_sh and git_sh.exists() else [env.get("ComSpec") or r"C:\Windows\System32\cmd.exe", "/c", command]
        result = subprocess.run(
            shell_command,
            cwd=context.root,
            capture_output=True,
